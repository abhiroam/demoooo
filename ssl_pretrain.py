...
class JointSSLEEGDataset(Dataset):
    def __init__(self, graph_dir: str):
        ...
        print(f"🔍 Found {len(files)} potential node files. Checking for concurrent writes...")
        valid_files = 0
        skipped = 0
        ...
        for f in files:
            ...
            adj_path  = f.replace("_nodes.npy", "_adj.npy")
            mask_path = f.replace("_nodes.npy", "_mask.npy")
            
            if not os.path.exists(adj_path) or not os.path.exists(mask_path):
                skipped += 1
                continue

            try:
                arr = np.load(f, mmap_mode="r")
                if arr.ndim != 4 or arr.shape[1] != NUM_WINDOWS: 
                    skipped += 1
                    continue
                n_rows = arr.shape[0]

                arr_adj = np.load(adj_path, mmap_mode="r")
                if arr_adj.shape[0] != n_rows: 
                    skipped += 1
                    continue
        ...
        print(f"✅ Successfully loaded {valid_files} completely written files.")
        if skipped > 0:
            print(f"⚠️ Skipped {skipped}/{len(files)} files (mid-write, corrupted, or missing channels).")
            
        if len(self.samples) == 0:
            raise RuntimeError("❌ No valid samples found after filtering. Pipeline cannot start.")
            
        print(f"📊 SSL Dataset Size: {len(self.samples)} sequences ready for training.")
...
def ssl_pretrain():
    ...
    start_epoch = 0
    p4_ckpt  = os.path.join(OUTPUT_DIR, "transformer_ssl_pretrained.pth")
    p23_ckpt = os.path.join(OUTPUT_DIR, "p23_ssl_pretrained.pth")
    opt_ckpt = os.path.join(OUTPUT_DIR, "ssl_optimizer.pth")

    if os.path.exists(p4_ckpt) and os.path.exists(p23_ckpt):
        p4.load_state_dict(torch.load(p4_ckpt, map_location=device))
        p23.load_state_dict(torch.load(p23_ckpt, map_location=device))
        if os.path.exists(opt_ckpt):
            ckpt = torch.load(opt_ckpt, map_location=device)
    ...
    if start_epoch >= SSL_EPOCHS:
        print(f"✅ SSL already completed {SSL_EPOCHS} epochs. Skipping training.")
        return

    for ep in range(start_epoch, SSL_EPOCHS):
        p23.train(); p4.train()
        totals = {"order": 0., "pred": 0., "contrast": 0., "total": 0.}
        n_steps = 0
        
        # Original Linear Decay (10-epoch warmup to stable 0.07)
        ...
        for step, (orig, shuf, neg) in enumerate(loader):
            ...
            totals["order"] += info["order"]
            totals["pred"] += info["pred"]
            totals["contrast"] += info["contrast"]
            n_steps += 1
        ...
        avg_total = totals["total"] / max(n_steps, 1)
        avg_order = totals["order"] / max(n_steps, 1)
        avg_pred  = totals["pred"] / max(n_steps, 1)
        avg_cont  = totals["contrast"] / max(n_steps, 1)
        ...
        if avg_total < best_loss:
            best_loss = avg_total
            torch.save(p4.state_dict(), p4_ckpt)
            torch.save(p23.state_dict(), p23_ckpt)
            
            print(f"Epoch {ep+1:2d}/{SSL_EPOCHS} | loss={avg_total:.4f} | "
                  f"order={avg_order:.3f} | pred={avg_pred:.4f} | "
                  f"contrast={avg_cont:.3f} | tau={current_tau:.3f}  ★ best")
        else:
            print(f"Epoch {ep+1:2d}/{SSL_EPOCHS} | loss={avg_total:.4f} | "
                  f"order={avg_order:.3f} | pred={avg_pred:.4f} | "
                  f"contrast={avg_cont:.3f} | tau={current_tau:.3f}")
